...
```powershell
python tests\run_all.py          # must be 12/12
```
...
## The wave pipeline — every wave, no exceptions
...
| 6 | **Test writer** | 1 | durable regression tests that provably fail when the guarded thing breaks — demonstrated by breaking it and showing the catch. |

≈9–10 agents per wave.

**Why the plurals are load-bearing.** Multiple recons because one angle misses
things and two agreeing independently is real evidence — the CPU pack format
...
## Two unknowns that cannot be resolved by reading

Both block planetary geometry. Both need an empirical run of the original.

1. **`random()` argument type.** Borland's `random()` is a textual macro, so a
   `double` argument keeps its type and the whole expression evaluates in
   double before truncation. LR casts to `int16_t` first. Not equivalent, and
   for large stars the cast can wrap. This is in the first loop of planet
   generation.
2. **`zrandom` evaluation order.** It is `random(r) - random(r)`, and operand
   order is unsequenced in C++. If Borland evaluated right-to-left, every
   `zrandom` returns the negation of what LR computes — flipping planet tilts
   and shifting every radius. Draw counts are unaffected, so planet types and
   counts survive; geometry does not.
...
| Facility | Result |
|---|---|
| clear + palette-expand + RETRACE, 320×200 | **0.799 ms — 1.5% of a 55 ms tick** |
| unthrottled frame rate | 785 fps (RETRACE does not block on vsync) |
| median tick, busy-wait on the HPT | **55.0000 ms**; p90 within 0.1 µs |
...
| exclusive 320×200 | real mode switch, closest thing to mode 13h |
...
**Audio: VOC feasible, MIDI not.** One loop buffer with a live playback cursor
...
decision** — one item per unit (4× memory, simple, fast, and now trivially
...
**latitudes -60 to +59** — 60 rows of the 360x180 texture are never shown.
...
needs no `*%` (the multiply is 32×32→32 low half only), and it gates 346
