...
def transform(original):
    ...
    replace_once(
        ...
        lines(
            b'"VHG local render"',
            b"\tA = [VHGlocalactive]; ? A = 0 -> VHG local render done;",
            b"\t( Attribution star phase 0: selected origin and resident scan. )",
    ...
    replace_once(
        ...
        lines(
            ...
            b"\t=> VHG fpu clean;",
            b"\t( Attribution star phase 1: primary-star setup and companion coronas. )",
            b"\t( The primary star is -planet_xyz-ship. )",
        ),
    )
    replace_once(
        lines(
            b"\t=> VHG local companion coronas;",
            b"\t( lens_flares_for's primary-star call receives the near star's own",
        ),
        lines(
            b"\t=> VHG local companion coronas;",
            b"\t[Timer Command] = READ COUNTS; isocall; A = [Counts]; A - [VHGprofauxstart];",
            ...
            b"\t=> VHG fpu clean;",
            b"\t( Attribution star phase 2: mask, primary-star globe or far pixel. )",
            b"\t( lens_flares_for's primary-star call receives the near star's own",
        ),
    )
    replace_once(
    ...
    replace_once(
        ...
        lines(
            b"\t( Attribution profile: space=local total, cupola=origin/scan, hull=star/coronas, detail=mask/star raster. )",
...
if __name__ == "__main__":
    ...
    print(f"star_instrumented_source_sha256={digest(SOURCE)}")
