...
assert "[sqstep] = srd3; A = [srd3]; [sqmh] = A; [srd3] = 0;" in candidate_root
assert "A = [sqmh]; E = A;" in candidate_root
...
assert "A = [srd3]; A < 2; B = [srd2]" not in candidate_root
assert "A = [srd2]; A < 2; B = [srd1]" not in candidate_root
assert "A = [srd1]; A < 2; B = [srd0]" not in candidate_root
...
assert candidate_root.endswith(
    "\tend;\n\t( The buffered handoff consumes the accepted calibration "
    "footprint. )\n\n")
...
for mantissa in mantissas:
    for exponent in (0, 1):
        ...
        assert (old_root == direct_root == word_root == buffered_root
                == math.isqrt(radicand))
        ...
        assert accepted_private == buffered_private == limb_private
        accepted_rounded = rounded_p64_root(word_root, accepted_private)
        candidate_rounded = rounded_p64_root(buffered_root, buffered_private)
        ...
        max_remainder_bits = max(
            max_remainder_bits, remainder_bits, buffered_bits)
        generic_cases += 1
...
for bits in f64_cases:
    ...
    buffered_pipeline = sqrt_pipeline(bits, buffered_compatible_word_root)
    ...
    assert accepted_pipeline == buffered_pipeline
    assert accepted_pipeline[0] == mathematical[0]
    assert buffered_pipeline[1] == rounded_p64_root(
...
for _ in range(4_096):
    ...
    output = sqrt_pipeline(bits, buffered_compatible_word_root)[0]
