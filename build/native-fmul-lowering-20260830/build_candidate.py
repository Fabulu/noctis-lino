...
assert digest(ACCEPTED / "compiler114m.exe") == apply[
...
assert digest(ACCEPTED / "fpabi.txt") == apply["ACCEPTED_FPABI_SHA256"]
...
stage_hashes = []
try:
    # The accepted compiler ignores the zero-byte marker byte-exactly.
    ...
    # Three self-host stages establish a byte-identical compiler fixpoint.
    ...
    stage_hashes = [
        digest(EVIDENCE / f"compiler-stage{stage}/compiler114m.exe")
        for stage in range(1, 4)]
    assert len(set(stage_hashes)) == 1, stage_hashes
    ...
    # Both compilers see the identical marker-bearing common source on x64.
...
assert digest(WORK_FP) == apply["ACCEPTED_FP_SHA256"]
assert digest(WORK_GAME) == apply["ACCEPTED_GAME_SHA256"]
assert len(stage_hashes) == 3 and len(set(stage_hashes)) == 1
report = {
    "schema": 1,
    "task": 235,
    "status": "pass",
    ...
    "non_i386m_cpu": "x64",
    "non_i386m_output_bytes": (
        EVIDENCE / "candidate-compiler-x64/vhgame.exe").stat().st_size,
