...
def pointer_word_root(radicand):
    ...
    iterations = 0
    boundary_advances = 0
    while limbs_remaining:
        pair = limbs[active] >> 30
        limbs[active] = (limbs[active] << 2) & MASK32
...
def pointer_compatible_word_root(radicand):
    root, mathematical_residual, remainder_bits = pointer_word_root(radicand)
    return (root, compatible_residual(root, mathematical_residual),
            remainder_bits)
...
assert '"XRoot restoring loop"' in accepted_root
assert '"XRoot restoring loop"' in candidate_root
assert accepted_root.count("[sqstep]+;") == 1
assert "? [sqstep] < 64 -> XRoot restoring loop;" in accepted_root
assert "[sqmh]" not in accepted_root and "[sqml]" not in accepted_root
assert "[sqmh] = srd3; [sqml] = 16; [sqstep] = 4;" in candidate_root
assert "B = [sqmh]; A = [B]; E = A;" in candidate_root
assert "A < 2; [B] = A;\n\tA = E; A > 30; E = A;" in candidate_root
assert candidate_root.count("[sqml]-;") == 1
assert "? [sqml] != 0 -> XRoot restoring loop;" in candidate_root
assert candidate_root.count("[sqstep]-;") == 1
assert "? [sqstep] = 0 -> XRoot restoring complete;" in candidate_root
assert "[sqmh]-; [sqml] = 16;" in candidate_root
assert '"XRoot restoring complete"' in candidate_root
candidate_loop = candidate_root[candidate_root.index(
    '"XRoot restoring loop"'):candidate_root.index(
        '"XRoot restoring complete"')]
assert "[srd3]" not in candidate_loop
assert "[srd2]" not in candidate_loop
assert "[srd1]" not in candidate_loop
assert "[srd0]" not in candidate_loop
...
for mantissa in mantissas:
    for exponent in (0, 1):
        ...
        pointer_root, pointer_residual, pointer_bits = pointer_word_root(radicand)
        assert (old_root == direct_root == word_root == pointer_root
                == math.isqrt(radicand))
        assert (mathematical_residual == direct_residual == word_residual
                == pointer_residual)
        assert word_root * word_root + word_residual == radicand
        assert pointer_root * pointer_root + pointer_residual == radicand
        assert word_residual < 2 * word_root + 1

        accepted_private = compatible_residual(word_root, word_residual)
        pointer_private = compatible_residual(pointer_root, pointer_residual)
        limb_private = accepted_limb_residual(radicand, old_root)
        assert accepted_private == pointer_private == limb_private
        accepted_rounded = rounded_p64_root(word_root, accepted_private)
        candidate_rounded = rounded_p64_root(pointer_root, pointer_private)
        ...
        max_remainder_bits = max(
            max_remainder_bits, remainder_bits, pointer_bits)
        generic_cases += 1
...
for bits in f64_cases:
    accepted_pipeline = sqrt_pipeline(bits, restoring_compatible_word_root)
    pointer_pipeline = sqrt_pipeline(bits, pointer_compatible_word_root)
    ...
    assert accepted_pipeline == pointer_pipeline
    assert accepted_pipeline[0] == mathematical[0]
    assert pointer_pipeline[1] == rounded_p64_root(
...
for bits in special_inputs:
    for xrej in (0, 1, MASK32):
        ...
        candidate_special = special_sqrt(
            bits, xrej, pointer_compatible_word_root)
...
for _ in range(4_096):
    ...
    output = sqrt_pipeline(bits, pointer_compatible_word_root)[0]
...
result = {
    ...
    "baseline_radix_limb_shifts_per_positive_root": 256,
    "candidate_radix_limb_shifts_per_positive_root": 64,
    "candidate_dynamic_limb_pointer_reads_per_positive_root": 64,
