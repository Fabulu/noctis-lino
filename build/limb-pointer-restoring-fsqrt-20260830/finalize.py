...
EXPECTED = {
    ...
    "candidate_fp": "62401cb0446e44fa4c4f93d51b11088aa25e7f898830a17c5265a37e60da30fb",
    "accepted_exe": "c4a62f5068262239a8a5665c443a75784fa2472941c9dfdb8fb731f5c8217ca2",
    "candidate_exe": "9844a08f0fc0d322eac8240d8efa58a9fb20d37872420d8f07cc24751b33e580",
...
def arm_result(name):
    ...
    expected_exe = EXPECTED[f"{kind}_exe"]
    ...
    assert report["provenance"]["executable_sha256"] == expected_exe
...
assert digest(ROOT / "work/fp/fpsoft.txt") == EXPECTED["accepted_fp"]
assert digest(ROOT / "work/vhgame.exe") == EXPECTED["accepted_exe"]
...
assert arms["candidate-a"]["simulation_gate_pass"]
assert arms["candidate-b"]["simulation_gate_pass"]
...
assert ordering_b["presentation_hz_delta"] < 0
assert ordering_b["cycles_per_presentation_delta"] > 0

result = {
    "schema": 1,
    "task": 223,
    "date": "2026-08-30",
    "decision": "rejected-after-contradictory-orderings-restored",
    "reason": (
        "Ordering A improved both mandatory metrics, but Ordering B independently lost "
        "both presentation throughput and cycles per presentation. The orderings were not "
        "averaged; fidelity and sustained screens were skipped, and Task #222 was restored."),
    "source_boundary": {
        "implementation": (
            "consume the restoring root radicand through one dynamic shared-Lino limb pointer"),
    ...
    "candidate": {
        "description": (
            "retain 64 restoring decisions while shifting one active 32-bit radicand limb "
            "per pair instead of shifting all four limbs"),
    ...
    "focused_gates": {
        ...
        "candidate_dynamic_limb_pointer_reads": model[
            "candidate_dynamic_limb_pointer_reads_per_positive_root"],
        "candidate_limb_boundary_advances": model[
            "candidate_limb_boundary_advances_per_positive_root"],
        ...
        "unreachable_padding_bytes": layout["unreachable_footprint_padding_bytes"],
    ...
    "fidelity": {
        "status": "skipped",
        "reason": "binding dual-ordering performance admission failed in Ordering B",
    },
    "sustained_screen": {
        "status": "skipped",
        "reason": "candidate was rejected before retention",
    },
    "host_classification": "controlled variable-host ABBA rejection",
...
for relative in manifest_paths:
    ...
    assert path.is_file()
    files.append({
        "path": (Path("build/limb-pointer-restoring-fsqrt-20260830") / relative).as_posix(),
...
manifest = {
    ...
    "files": files,
    "active": {
        "work/fp/fpsoft.txt": EXPECTED["accepted_fp"],
        "work/vhgame.exe": EXPECTED["accepted_exe"],
    },
    "current_files_match_accepted": True,
}
...
print(json.dumps({
    ...
    "ordering_a": ordering_a,
    "ordering_b": ordering_b,
    "restored_production": result["restored_production"],
}, indent=2))
