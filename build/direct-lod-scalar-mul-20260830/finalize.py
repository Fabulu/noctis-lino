...
assert candidate_source_sha256 == "ad8a1a96223f77a9df9073cc8b55073ee04664ca356dd5747ba2e54ce0490172"
assert candidate_executable_sha256 == "948343858dfa58d163e0459f56b59c33ff731642af26bb2960a61ddcd6f1d3c1"
...
assert model["status"] == "pass" and model["replacement_sites"] == 6
assert model["candidate_file_equals_exact_transform"]
assert model["generated_radius_zrandom_bounds"] == [-99, 99]
assert abs(model["generated_ray_lower_bound"] - 0.002727) < 1e-18
assert model["p64_then_p53_products_exact"]
assert not model["empirical_cross_host_execution_claimed"]
assert model["first_fi_observer_preceded_by_fcmp"]
assert model["fs0_and_b_through_e_exact"]
...
assert production_layout["replacement_sites"] == 6
assert production_layout["package_bytes_outside_six_islands_exact"]
assert production_layout["unexpected_changes"] == 0
assert production_layout["external_direct_entries_to_site_interiors"] == 0
assert production_layout["int_to_f_calls_removed"] == 6
assert production_layout["portable_fmul_calls_removed"] == 6
assert production_layout["direct_x87_p64_spill_sequences"] == 6

arm_kinds = {
    "candidate-a": candidate_executable_sha256,
    "baseline-a": accepted_executable_sha256,
    "baseline-b": accepted_executable_sha256,
    "candidate-b": candidate_executable_sha256,
}
metrics = {}
for arm, executable_sha256 in arm_kinds.items():
    report = load(f"{arm}/capsule/report.json")
    validate_arm(report, executable_sha256)
    metrics[arm] = arm_metrics(report)
...
result = {
    "schema": 1,
    "task": 220,
    "candidate": "replace six fixed LOD IntToF plus portable FMul sequences with direct shared-Lino p64-then-p53 scalar multiplies",
    ...
    "performed_arms": ["candidate-a", "baseline-a", "baseline-b", "candidate-b"],
    ...
    "fidelity": {"status": "skipped_after_contradictory_ordering_b"},
    "sustained_candidate": {"status": "skipped_after_contradictory_ordering_b"},
    "sustained_native_60_hz_status": "still_open",
    "disposition": "rejected_after_contradictory_abba_orderings",
