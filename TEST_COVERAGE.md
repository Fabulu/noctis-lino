...
| Linux AArch64 runtime and compiler target | `test_aarch64_runtime.py` bootstraps `compiler114m.txt` to an i386m fixpoint, packs the checked runtime as an AArch64 SYS, and compiles a real Lino source without a CPU pack; that image executes moves, workspace access, wrapping arithmetic, bitwise/logical/arithmetic shifts, equality, signed/unsigned comparisons, bit tests, control flow, calls, and full-width isocalls above 4 GB, including register/direct left operands and immediate/register/direct right operands, while independent ABI fixtures retain seven malformed-image refusals; all 12 checks passed under QEMU in hosted run 32567111729 | This proves the direct-workspace integer slice and Linux runtime bridge, not indirect operands, multiply/divide, unary/rotate or floating-point coverage, a macOS application, or a native ARM64 Noctis build |
