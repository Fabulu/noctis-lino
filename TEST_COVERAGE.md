...
| Planet classes, terrain, water, weather and suns | All accepted class arms, terrain bounds, daylight, secondary suns, reflections, waves and storms; `tests/test_sun_gallery.py` pins the retained type-3/type-5 BMP, surface state, exact active palette, complete palette-band page, camera, flare admission, and solar diagnostics without claiming unavailable snapshot-time low-six-bit indices; `tests/test_surface_flare_oracle.py` separately requires ten exact primitive pages | Lunar/dense/thin/frozen gallery plus dry-cell sun scene; native sessions in `PLAYTEST.md`. A canonical private-Windows thin-world trace retains centre index 126 on the completed flare page, then localizes the product's final 125 to the exact two source `psmooth_64` passes (22,537 changed indices) before a border pass that leaves the centre unchanged; this closes the false flare/border attribution while keeping the unretained final low-six-bit state informational |
