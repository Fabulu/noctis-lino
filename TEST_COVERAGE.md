...
| Orbital primary flares | `tests/test_orbitprimary_oracle.py` pins WIRE's authentic one-body class-7 system, untargeted exterior camera, strict `6*ray < distance < 1000*ray` source gate, immutable native BMP/page/palette hashes, and explicit non-atomic snapshot limit | Matched native/product WIRE evidence retains 148/136 bright low-six flare-core pixels, the exact 192-component space-palette band, and all 4,256 native palette bands in `(120,60)..(195,115)`; complete-page and exact low-six equality remain intentionally unclaimed |
