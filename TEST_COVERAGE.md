...
| Linux AArch64 runtime and compiler target | `test_aarch64_runtime.py` bootstraps `compiler114m.txt` to an i386m fixpoint, packs the checked runtime as an AArch64 SYS, and compiles a real Lino source without a CPU pack; that image executes moves, workspace access, wrapping add/subtract and low-word multiply, signed/unsigned division and remainder, bitwise/logical/arithmetic shifts, equality, signed/unsigned comparisons, bit tests, control flow, calls, and full-width isocalls above 4 GB, including register, direct-workspace, and canonical indirect-workspace left operands and immediate/register/direct/indirect right operands; indirect forms preserve pointer registers, source-before-destination order, and memory writeback semantics, while independent ABI fixtures retain seven malformed-image refusals; all 12 checks passed under QEMU in hosted run 32569243601 | This proves the multiply/divide/remainder and indirect-workspace integer slice plus Linux runtime bridge, not unary/rotate or floating-point coverage, a macOS application, or a native ARM64 Noctis build |
