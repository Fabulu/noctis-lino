...
- The complete macOS app smoke is proven on Apple Silicon through Rosetta 2.
  ...
  exact FP exception-state compatibility, exact x87 trigonometric range reduction
  below `2^63` and observable C2/status output, x87 FPREM widths other than the
  measured reference, and remaining exception/transcendental semantics; no native
  macOS runtime, Cocoa application, or native ARM64 Noctis
  build exists yet.
