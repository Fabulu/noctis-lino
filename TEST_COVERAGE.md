...
|---|---|---|
| Stardrifter, movement, roof lift and cupolas | `tests/test_vhgame.py` pins controls, lift states, cupola apertures, hull order, focus-safe repaint and resize, the four source smoothing rates and formatting for the lower environmental row, its `draw_hud` gate, and source-ordered in-place diffusion at all four visor lamps; `tests/test_orbitlunar_oracle.py` pins the strict one-unit interior/roof branch and the 69-pixel normalized `GRAVITY`/lamp-fringe mask | Retained native inside/roof captures plus two private-desktop product pairs prove the product's 28-by-5 environmental crop is byte-identical to native in both modes; interior and through-window stellar-flare screenshots, resize and movement sessions remain in `PLAYTEST.md`. Whole-row numerical-state equality is not claimed because the retained runs have different smoothing histories. |
