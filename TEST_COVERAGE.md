...
| Planet classes, terrain, water, weather and suns | All accepted class arms, terrain bounds, daylight, secondary suns, reflections, waves and storms; `tests/test_sun_gallery.py` pins nine retained type-1/2/3/4/5/7/8 BMPs and surface states (including separate class-0 and class-1 type-1 and type-4 cases), indexed-page/palette hashes, projected centres, and reproducible checkpoints, then applies case-specific exact palette, complete palette-band, upper-sky index/band crop, camera, flare-admission, and solar-state authority across positive, lower-suppressed, and upper-suppressed ray/distance gates; `tests/test_surface_flare_oracle.py` separately requires ten exact primitive pages | Two-lunar/dense/habitable/two-rocky/thin/frozen/quartz native pairs plus the dry-cell positive sun scene; native sessions in `PLAYTEST.md`. The hosted Apple-Silicon product job rebuilds and grades all nine surface checkpoints independently. The type-8 quartz pair additionally pins the exact final centre index, all 768 palette components, and 36,000 upper-sky palette bands at a certified native clock. The class-1 rocky pair authenticates the positive gate against the class-0 rocky upper-suppression case and pins centre `(161,72)`, final index 126, all 768 palette components, and all 27,000 upper-sky indices at source time `1345723230`, with `10 * 21.879 <= 245.8964 < 1000 * 21.879`. The class-1 lunar pair authenticates the positive gate against the class-0 lunar lower-suppression case and pins centre `(161,91)`, all 768 palette components, and all 64,000 final-page palette bands at source time `1345636830`, with `10 * 21.879 <= 1757.4972 < 1000 * 21.879`. A canonical private-Windows thin-world trace retains centre index 126 on the completed flare page, then localizes the product's final 125 to the exact two source `psmooth_64` passes (22,537 changed indices) before a border pass that leaves the centre unchanged; this closes the false flare/border attribution while keeping unsupported whole-page and low-six-bit contracts informational |
