...
Two rules from the rest of the suite are kept here:
  ...
  * TESTS NEVER TOUCH work/. Every run copies the five engine sources -
    fpabi.txt, test-only fpctlx87.txt (renamed fpctl.txt in the sandbox),
    fpx87.txt, fpsoft.txt and fpchains.txt - out of work/fp into a fresh sandbox
    under tests/gen and builds there. The code under test is still the code in
    work/fp: a regression there fails this test. But work/fp keeps whatever the
...
LIBS = (
    ("fpabi.txt", "fpabi.txt"),
    ("fpctlx87.txt", "fpctl.txt"),
    ("fpsoft.txt", "fpsoft.txt"),
    ("fpx87.txt", "fpx87.txt"),
)
SCHED = "fpsched.txt"
CHAINS = "fpchains.txt"
...
BATTERIES = [
    ...
    ("NsIdentity, deliberately perturbed ambient CW", "break"),
...
def break_nocw(text):
    """Delete every ``fldcw [FCW]`` from the test-only x87 control layer.

    The probe first records the runtime word, then deliberately installs a
    wrong ambient word with its own private x87 witness before calling FEnter.
    Removing both loads therefore proves that merely documenting FCWEXT cannot
    repair a hostile caller environment, independently of what the platform
    runtime supplied at process entry. The probe reports the forced word, so
    the referee predicts this break from a measurement rather than a platform
    assumption.
    """
...
class Probe(object):
    ...
    def install_sources(self):
        ...
        for source_name, target_name in LIBS:
            shutil.copyfile(os.path.join(FPWORK, source_name),
                            os.path.join(self.dir, target_name))
...
TEMPLATE = r"""      ( *** %(stem)s - GENERATED by tests/fpdrive.py, do not edit ***
	...
	%(hdr)d header units then %(nbat)d x %(nstar)d doubles.
	  ...
	  2  a deliberately hostile ambient control word - FEnter's saved
	     one, reinstalled. Header slot 2 says what that word was; slot
	     12 records the platform runtime word before the test perturbs it.
...
"programme"
	...
	( Record the platform boundary before this historical x87 test
	  deliberately creates a hostile caller environment.  The private
	  raw load is test evidence, never a production dependency. )
	=> FCWRead;
	A = [FI];		[fchead plus 12] = A;
	[FCW] = 1E3Fh;		(PC=53, RC=chop)
	{
