...
def main() -> int:
    ...
    check(numbers == list(range(22, 8, -1)),
          "release-note headings are unique and ordered Beta 22 through Beta 9")
    ...
    for bad in ("v0.1.0", "v0.1.0-beta.0", "v0.1.0-beta.23"):
        try:
    ...
    with tempfile.TemporaryDirectory(prefix="noctis-release-notes-") as directory:
        ...
        process = subprocess.run(
            [sys.executable, str(SCRIPT), "v0.1.0-beta.22", "--output", str(output)],
            cwd=ROOT,
            text=True,
