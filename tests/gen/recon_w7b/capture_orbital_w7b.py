...
def stage(current: Path, surface: Path | None = None) -> list[tuple[Path, Path]]:
    if BACKUP.exists():
        raise RuntimeError(
    ...
    try:
        ...
        if surface is not None:
            shutil.copy2(surface, DATA / "SURFACE.BIN")
        GALLERY.mkdir()
    except Exception:
        restore(saved)
        raise
...
def main() -> int:
    ...
    parser.add_argument(
        "--surface", type=Path,
        help="optional complete 40-byte SURFACE.BIN for a landed capture",
    )
    ...
    surface = args.surface.resolve() if args.surface is not None else None
    ...
    if surface is not None and (not surface.is_file() or surface.stat().st_size != 40):
        parser.error("--surface must name a complete 40-byte SURFACE.BIN")
    ...
    surface_path = HERE / "out" / (args.name + ".SURFACE.BIN")
    ...
    try:
        ...
        if surface is not None:
            command_line.extend(["-PreserveSurface", str(surface_path)])
        ...
        if surface is not None and (
                not surface_path.is_file() or surface_path.stat().st_size != 40):
            raise RuntimeError("native SURFACE.BIN was not preserved completely")
